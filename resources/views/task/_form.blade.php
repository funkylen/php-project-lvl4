<x-app-layout>
    ...
    <div class="mb-3">
        ...
        <select id="assigned_to_id" name="assigned_to_id"
            ...
            @foreach($assignees as $assignee)
                <option
                    value="{{ $assignee->id }}" @selected($assignedToId == $assignee->id)>{{ $assignee->name }}</option>
            @endforeach
        </select>
    ...
    <div class="mb-3">
        ...
        <select multiple id="labels" name="labels[]"
                class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
            @foreach($labels as $label)
                <option
                    value="{{ $label->id }}" @selected($taskLabelsIds->contains($label->id))>{{ $label->name }}</option>
            @endforeach
        </select>
